...
void get_peer_information(int fd);


// prints IP address and port of given fd
void print_peer_information(int fd) {
    struct sockaddr_in peer;
    socklen_t len = sizeof(struct sockaddr);
    if(getpeername(fd, (struct sockaddr *) &peer, &len) == -1) {
        perror("failure getting peer information");
    }
...
void send_message2client(char* header, int  new_fd, int headerlength, uint16_t _keylen, char* _key, uint32_t _valuelen, char* _value){
    ...
    if(_requested_del > 0){
        ...
        if(result == NULL){
            ...
            //write(1, result->value, result->value_length);
        }
    }
}
